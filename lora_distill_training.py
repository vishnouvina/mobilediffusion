...
def main():
    ...
    if accelerator.is_main_process:
        if args.use_peft:
            ...
            state_dict = get_peft_model_state_dict(unwrapped_unet, state_dict=accelerator.get_state_dict(unet))
            lora_config["peft_config"] = unwrapped_unet.get_peft_config_as_dict(inference=True)
            if args.train_text_encoder:
                unwrapped_text_encoder = accelerator.unwrap_model(text_encoder)
                text_encoder_state_dict = get_peft_model_state_dict(
                    unwrapped_text_encoder, state_dict=accelerator.get_state_dict(text_encoder)
                )
                text_encoder_state_dict = {f"text_encoder_{k}": v for k, v in text_encoder_state_dict.items()}
                state_dict.update(text_encoder_state_dict)
                lora_config["text_encoder_peft_config"] = unwrapped_text_encoder.get_peft_config_as_dict(
                    inference=True
                )
